...
the shared vocabulary.
  ^^^ [RETRACTED 2026-07-27 — FALSE. See the banner at the top of this file: cb_only shares only
      the codebook, so the common tokenizer this sentence assumes does not exist. The
      "shared vocabulary" premise of line 7 above falls with it, and `onboard_mixture` is
      WITHDRAWN outright (RESEARCH_LEDGER.md, Group 4); `onboard_count` survives only as a
      fiat-tokenizer probe, internally valid and externally mis-specified.]
...
def main() -> int:
    ...
    if not all_records:
        raise SystemExit(
            "[onboard] produced 0 records — refusing to write an empty ledger.\n"
