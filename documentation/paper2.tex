...
We summarize only what the federation design needs; for the method itself we refer to the original
paper~\cite{lee2024timevqvaead}. Throughout, a \emph{client} owns a set
of windows of a univariate series and never shares them.
...
\caption{Configuration (g) on client $k$. Stage~1 federates the encoder and its BN running
statistics and keeps the broadcast codebook fixed within a round, the server updating it
from~\eqref{eq:suffstat}; the decoder is never sent. Stage~2 shares the body of the MaskGIT prior,
keeping the channel embedding and output bias local. Inference is local and forks into the anomaly
score and the counterfactual.}
\label{fig:framework}
\end{figure}
...
\begin{equation}
    ...
    \label{eq:suffstat}
\end{equation}
with $\tilde{N}_k$ a Laplace-smoothed count, so that each global codeword is updated from statistics
accumulated across clients without transmitting latent samples or raw data. This motivates the
...
tokenizer, since the encoder keeps changing.
...
\begin{table}[t]
\caption{Configurations evaluated. ``FedAvg'' is count-weighted parameter averaging, ``pooled'' the
aggregation of BN statistics, ``suff.\,stat.'' the codebook update of~\eqref{eq:suffstat},
``partial'' the shared prior with client-specific components local. $n$ = development series run
and, in parentheses, coverage of the four that separate the arms at all
(Section~\ref{subsec:discriminating}), so $4/4$ is a complete cover.}
\label{tab:space}
\centering
\scriptsize
...
needed. No training is shared, and the profiles can be averaged only because the split divides the
training segment while the test segment is common, so they are defined on the same timesteps; all
five models must also be present at inference. Neither holds where clients score their own streams.
...
\caption{One development series, \texttt{ucr\_001}, under configuration (g). (a)~the training
segment is cut chronologically into five client shards of $10/10/20/20/30\%$ plus a validation tail
(v); the test segment and its labels are shared and never seen in training. (b)~each client's
per-timestep score, its own train-fitted threshold (dashed), the timesteps it declares anomalous
(tinted) and the single location it returns under the archive protocol (marker): all five agree,
inside the labelled event, while their thresholds admit $1.2$ to $4.1\%$ of the test timesteps.
(c)~the same event with the explanation, built with \emph{no label in the loop}: the timesteps above
client~1's threshold are masked across frequency, resampled from the prior with the remaining tokens
as context and decoded locally (band = five draws, solid = median). Every configuration already
localizes this series.}
\label{fig:overview}
\end{figure*}

Two settings are inherited from the original method rather than tuned, since it states them as
such: the window length is $T=2P$, twice the measured period of the series, and each input window is
z-normalized before the transform; we also keep its frequency resolution ($H=3$ bands) and latent
temporal width ($32$). With $T=2P$ the UCR Time Series Anomaly archive~\cite{wu2023ucr} leaves 180
usable series: the remainder either have no measured period or leave the smallest client
with fewer samples than one window. Ten form a development set on which the configurations were
explored, selected by period and domain rather than by detection performance, the period-aligned
window length being approximately logarithmically spaced from $182$ to $970$ samples.
...
All configurations use the same trainer and select the best validation round, but the stopping
criterion is granular to the setting: local and centralized training stop on validation patience
counted in optimizer steps inside a fixed step budget, federated configurations on patience counted
in aggregation rounds, validation being defined once per round; a federated run that exhausts its
round ceiling without triggering patience is marked truncated and excluded. The baselines' step
budget binds in $9\%$ of their stage-runs on the development set and in $15$ to $25\%$ on the
confirmation sample, and those runs are retained, so the baselines are, if anything, the arm more
likely to be reported before convergence.

Capacity is fixed across configurations: a codebook of $K=64$ entries of dimension $64$, an encoder
of base width $16$, and a four-layer prior with four heads and width $128$, optimized with AdamW.
The codebook is smaller than in the original method, so our reference results are not directly
...
client the picture differs: centralized training exceeds it on 7 of 10 series in AUPRC ($p=0.34$),
...
(sign test $p=0.021$); against a baseline trained to a budget four to five times larger with early
...
aggregation, the same reference at $31$ local epochs per round with $\approx310$ local steps between
server updates instead of $100$ to $190$, costs $0.198$ and $0.245$ AUPRC on \texttt{ucr\_011} and
\texttt{ucr\_043}, so the default frequency is sufficient and increasing it buys nothing.

\begin{table}[t]
\caption{Development set. Left: archive accuracy at tolerance $64$; right: AUPRC of the per-timestep
score, with the parameter-free floor as leftmost reference. $\max_j$ is the best individual client
and requires test labels to select. The four discriminating series are above the rule. On
\texttt{ucr\_082} every arm is at floor: the counts in the text rest on differences in the fifth
decimal.}
\label{tab:main}
\centering
\scriptsize
...
a paired sign test with ties counted as ties, and AUPRC is secondary. Three configurations are run
...
\begin{table}[t]
\caption{Confirmation sample, $n=48$, paired by series. Primary endpoint: archive accuracy at
tolerance $64$; AUPRC secondary. $p$ is a two-sided sign test with ties excluded. The mean is given
alongside the median because on the primary endpoint the median is zero wherever ties dominate.}
\label{tab:c50}
\centering
\scriptsize
...
\begin{table}[t]
\caption{Counterfactual quality. $\Delta$ = plausibility gain on the rewritten positions, each arm
under its own prior; $G$ = specificity gap between anomalous and normal windows. Above the rule, the
four discriminating series.}
\label{tab:cf}
\centering
\scriptsize
\setlength{\tabcolsep}{2.2pt}
...
effects. Six of the ten development series cannot separate the arms at all, and that set was chosen
purposively, so no quantity measured on it estimates a population; that role belongs to the random
sample, whose primary endpoint proved too coarse to separate any pair of arms. Finally, the aligned
and unaligned branches of Section~\ref{subsec:interaction} were not run entirely on matched
hardware, (g) having run on one accelerator while (e) used two others for seven of the ten series: a
confound we can declare but not bound.
...
shared matters more than \emph{how} it is aggregated: replacing encoder FedAvg by FedProx, by
FedProto or by a control that shares only the initialization changes nothing measurable, while
changing the codebook rule changes a great deal on individual series and nothing in aggregate.
...
recovers the gap at no training cost, the diversity an ensemble exploits being exactly what
parameter averaging destroys. On the counterfactual, federation costs nothing we can
measure, which locates its price entirely in detection.
...
is also deliberately homogeneous, and two choices are expected to invert with heterogeneous clients:
pooling the BN statistics, adopted against the FedBN prescription~\cite{li2021fedbn} precisely
because no feature shift exists here, and a single shared codebook.

\section*{Data and Resources}
The data used in this study are in the \href{https://www.cs.ucr.edu/\%7Eeamonn/time_series_data_2018/UCR_TimeSeriesAnomalyDatasets2021.zip}{UCR Time Series Anomaly Archive}. The implementation is available at \texttt{[REPOSITORY URL]}.
% ⚠️ DA COMPILARE PRIMA DELLA SUBMISSION: sostituire \texttt{[REPOSITORY URL]} con il link vero,
% p.es. \href{https://github.com/<org>/<repo>}{github.com/<org>/<repo>}.
